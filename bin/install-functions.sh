...
confirm_jre()
{
    ...
    if [ -z "$JRE_FLAVOR" ]; then
        fail "Please confirm that the JRE in the PATH is Oracle's version."
    fi
}

# confirm version of rrdtool
confirm_rrd()
{
    # TODO: Would be good to autoconfize this so desired rrd version
    #       flows from hardcoded value in configure.ac.
    RRD_MINVER="1.4.7"

    echo -n "Checking RRDtool version >= ${RRD_MINVER}: "
    if ! which rrdtool 2>/dev/null 1>&2 ;then
        echo "[FAIL] Missing."
        fail "Error: rrdtool is not in the search path.  Please confirm that RRDtool (version >= ${RRD_MINVER}) is installed."
    fi

    RRD_BOXVER=`rrdtool | sed -e "s/^RRDtool \([0-9]*\.[0-9]*\.[0-9]*\) .*/\1/g" | head -1`
    case "${RRD_BOXVER}" in
        [0-9]*.[0-9]*.[0-9]*)
            if [ $(get_version ${RRD_BOXVER}) -lt $(get_version ${RRD_MINVER}) ]; then
                echo "[FAIL] ${RRD_BOXVER}"
                fail "Error: unsupported version of RRDtool. The installed version (${RRD_BOXVER}) is older than the minimum required ${RRD_MINVER}"
            else
		echo "[ OK ] ${RRD_BOXVER}"
            fi
            ;;
        *)
            if [ -z "${RRD_BOXVER}" ]; then
                echo "[FAIL]"
                fail "Error: unable to determine your RRDtool version. Please confirm that RRDtool (version >= ${RRD_MINVER}) is installed."
            else
                echo "[FAIL] ${RRD_BOXVER}"
                fail "Error: Encountered unexpected RRDtool version string (${RRD_BOXVER}).  Expecting #.#.#"
            fi
            ;;
    esac
} # end confirm_rrd()
...
configure_amqp() {
    ...
    if [ ! -z "$RABBITMQCTL" ]; then
        local user_exists=`"$RABBITMQCTL" -q list_users | awk '{print $1}' | grep '^'"$RABBITMQ_USER"'$'`
        if [ -z "$user_exists" ]; then
            echo "Adding RabbitMQ user: $RABBITMQ_USER"
            "$RABBITMQCTL" -q add_user "$RABBITMQ_USER" "$RABBITMQ_PASS"
        fi
        local vhost_exists=`"$RABBITMQCTL" -q list_vhosts | awk '{print $1}' | grep '^'"$RABBITMQ_VHOST"'$'`
...
run_mkzopeinstance()
{
    ...
    if [ ! -f backup.tar ] ; then
        ...
        $PYTHON $ZOPEHOME/mkzopeinstance \
            --dir="$ZENHOME" \
        ...
        chown root:$USERNAME $ZENHOME/bin/{pyraw,zensocket,nmap} 2>/dev/null
        if [ $? -ne 0 -a -n "${no_pw_prompt_opt}" ]; then
        sudo ${no_pw_prompt_opt} chown root:$USERNAME $ZENHOME/bin/{pyraw,zensocket,nmap} 2>/dev/null
        if [ $? -ne 0 ]; then
        echo "Make sure to execute:"
                echo "chown root:$USERNAME ${ZENHOME}/bin/{pyraw,zensocket,nmap}"
        fi
        fi
        chmod 04750 $ZENHOME/bin/{pyraw,zensocket,nmap} 2>/dev/null
        if [ $? -ne 0 -a -n "${no_pw_prompt_opt}" ]; then
            sudo ${no_pw_prompt_opt} chmod 04750 $ZENHOME/bin/{pyraw,zensocket,nmap} 2>/dev/null
            if [ $? -ne 0 ]; then
                echo "Make sure to execute: "
                echo "chmod 04750 ${ZENHOME}/bin/{pyraw,zensocket,nmap}"
            fi
        fi
...
run_zenbuild()
{
    if [ ! -f backup.tar ] ; then
        ...
        if [ "$(id -u)" != "0" ]; then
            $ZENHOME/bin/zenbuild $ZENBUILD_ARGS >> $ZENHOME/log/zenbuild_init.log 2>&1 || fail "Unable to create the initial Zenoss object database"
        else
            su $OS_USERNAME -l -c "$ZENHOME/bin/zenbuild $ZENBUILD_ARGS >> $ZENHOME/log/zenbuild_init.log 2>&1" || fail "Unable to create the initial Zenoss object database"
        fi
    else
        run_zenmigrate >> $ZENHOME/log/zenmigrate_init.log 2>&1 || fail "Data migration failed"
    fi
...
start_rabbitmq()
{
    ...
    do
        if [ -x ${script} ]; then
            if ${script} status 2> /dev/null 1>&2 ; then
               # rabbitmq already started
               return
            fi

            if ${script} start 2> /dev/null 1>&2 ; then
               # Success!
               return
            fi

	    # If we get here we probably have a broken rabbitmq or lack privileges
            fail "Failed starting rabbitmq -- exiting!"
        fi
    done

    # If we get here we probably don't have Rabbit MQ -- doh!
    fail "No startup script for Rabbit MQ found -- exiting!"
}
    ...
    if [ "`id -u`" = "0" ];then
        ...
        if [ -z "${owner}" ];then
            ...
            if id ${owner} 2>/dev/null 1>&2 ;then
                ...
                chown root:${group} ${ZENHOME}/bin/{zensocket,pyraw,nmap}
                chmod 04750 $ZENHOME/bin/{zensocket,pyraw,nmap}
            else
